...
import { convertWeiToETH, getETHPriceInUSD, getWEIPriceInUSD } from "@/lib/getETHPrice";
import useStore from "@/store";
import useSignerStore from "@/store/useSignerStore";
import { handleContribute } from "@/services/campaign.service";
...
export default function CampaignSingle() {
    ...
    const thisCampaign = campaigns.filter((camp) => camp.address?.slice(4, 24) == id)[0]

    async function onSubmit(data) {
        console.log(data);
        try {
            await handleContribute(signer, thisCampaign?.address, data.value, thisCampaign?.manager)
                ...
                <Box position={"relative"}>
                    ...
                    >
                        <Stack spacing={{ base: 6 }}>
                            ...
                            >
                                {thisCampaign?.name?.replace(/\b\w/g, (char) => char.toUpperCase())}
                            </Heading>
                            <Text
                                color={useColorModeValue("gray.500", "gray.200")}
                                fontSize={{ base: "lg" }}
                            >
                                {thisCampaign?.description}
                            </Text>
                            <Link
                                color="teal.500"
                            ...
                            <Box mx={"auto"} w={"full"}>
                                <SimpleGrid columns={{ base: 1 }} spacing={{ base: 5 }}>
                                    <StatsCard
                                        ...
                                        stat={`${convertWeiToETH(thisCampaign?.minimumContribution)} ETH ($${getWEIPriceInUSD(
                                            ethPrice,
                                            thisCampaign?.minimumContribution
                                        )})`}
                                        info={
                                    ...
                                    <StatsCard
                                        ...
                                        stat={thisCampaign?.requestsCount}
                                        info={
                                    ...
                                    <StatsCard
                                        ...
                                        stat={thisCampaign?.approversCount}
                                        info={
                        ...
                        <Stack spacing={{ base: 4 }}>
                                        ...
                                        >
                                            <Text as="span" fontWeight={"bold"}>
                                                {thisCampaign?.balance > 0
                                                    ? convertWeiToETH(thisCampaign?.balance)
                                                    : "0, Become a Donor 😄"}
                                            </Text>
                                            <Text
                                                as="span"
                                                display={thisCampaign?.balance > 0 ? "inline" : "none"}
                                                pr={2}
                                                fontWeight={"bold"}
                                            >
                                                {" "}ETH
                                            ...
                                            <Text
                                                ...
                                                color={useColorModeValue("gray.500", "gray.200")}
                                            >
                                                (${getWEIPriceInUSD(ethPrice, thisCampaign?.balance)})
                                            </Text>
                                        </Box>

                                        <Text fontSize={"md"} fontWeight="normal">
                                            target of {convertWeiToETH(thisCampaign?.target)} ETH ($
                                            {getWEIPriceInUSD(ethPrice, thisCampaign?.target)})
                                        </Text>
                                        <Progress
                                            colorScheme="teal"
                                            size="sm"
                                            value={thisCampaign?.balance}
                                            max={thisCampaign?.target}
                                            mt={4}
                                        />
                                    </StatNumber>
                                </Stat>
                            ...
                            <Stack
                                ...
                                spacing={4}
                            >
                                <NextLink href={`/campaign/requests/${id}`}>
                                    <Button
                                        fontFamily={"heading"}
                                        w={"full"}
                                        bgGradient="linear(to-r, teal.400,green.400)"
